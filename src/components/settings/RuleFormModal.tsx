...
export default function RuleFormModal({ rule, initialValues, onClose }: RuleFormModalProps) {
  ...
  const { conflictData, checkForConflict, resolveConflict, hasConflict, clearConflict } =
    useOptimisticLocking<Rule>(['rules', activeBookset?.id || '']);
    ...
    try {
      if (rule) {
        ...
        const updatedRule: Rule = {
          ...
          match_type: validation.data.matchType as MatchType,
          case_sensitive: validation.data.caseSensitive,
          target_category_id: validation.data.targetCategoryId,
          suggested_payee: validation.data.suggestedPayee || null,
        ...
        const hasConflict = await checkForConflict(rule, updatedRule);
        if (hasConflict) return; // Show conflict modal

        await updateRule(rule.id, validation.data);
      } else {
        await createRule(validation.data);
      }
      onClose();
    } catch (err) {
  ...
  const handleConflictResolve = async (strategy: 'overwrite' | 'reload') => {
    ...
    if (strategy === 'overwrite' && resolvedRecord && rule) {
      // User chose to keep their changes - force the update
      try {
        const resolvedConditions = (resolvedRecord.conditions as RuleConditions) || {};
        await updateRule(rule.id, {
          keyword: resolvedRecord.keyword,
      ...
      const serverConditions = (serverRecord.conditions as RuleConditions) || {};
      setFormData({
        ...
        amountMin: serverConditions.amountMin?.toString() || '',
        amountMax: serverConditions.amountMax?.toString() || '',
        descriptionRegex: serverConditions.descriptionRegex || '',
        startMonth: serverConditions.dateRange?.startMonth?.toString() || '',
        endMonth: serverConditions.dateRange?.endMonth?.toString() || '',
        startDay: serverConditions.dateRange?.startDay?.toString() || '',
        endDay: serverConditions.dateRange?.endDay?.toString() || '',
      });
    }
  };
    ...
    <>
      ...
      {hasConflict && conflictData && (
        <VersionConflictModal
          isOpen={hasConflict}
          entityType="rule"
          entityName={`Rule: "${conflictData.updatedRecord.keyword}"`}
