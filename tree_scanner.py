...
Default Excludes:
  Version Control: .git, .github, .codeql, .codeql-db
  Build/Dependencies: node_modules, dist, dev-dist, build, coverage, venv, __pycache__
  Caches: .vite, .vite-cache, .cache, .eslintcache, .ruff_cache, .pytest_cache
  Test Artifacts: test-results, playwright-report, e2e/.auth
  Config Files: package.json, package-lock.json, eslint.config.js, tsconfig.json, etc.
  Documentation: README.md, CHANGELOG.md, CLAUDE.md, agents.md, etc.
  (See script for complete list)

Default Line Counting: .js, .ts, .tsx, .html, .css (your actual code files)
"""
...
class TreeScanner:
    def __init__(self, 
        ...
        """
        # Default excludes - organized by category
        self.excludes = excludes or {
            # Version control & CI/CD
            ".git", ".github", ".codeql", ".codeql-db", "codeql-results.sarif",

            # Dependencies & build artifacts
            "node_modules", "dist", "dev-dist", "build", "coverage", "venv", "__pycache__",

            # Cache & temporary files
            ".vite", ".vite-cache", ".cache", ".eslintcache", ".ruff_cache", ".pytest_cache",

            # Test artifacts
            "test-results", "playwright-report", "e2e/.auth",

            # IDE & editor configs
            ".vscode", ".idea", ".claude", ".serena",

            # Config files (root level)
            ".env.example", ".gitignore", ".husky", ".markdownlint.json", ".markdownlintignore",
            ".npmrc", ".prettierignore", ".prettierrc.json", ".versionrc.json",
            "eslint.config.js", "knip.json", "package.json", "package-lock.json",
            "papas-books.code-workspace", "playwright.config.ts", "postcss.config.js",
            "tailwind.config.js", "tsconfig.json", "vite.config.js",

            # Documentation & notes
            "agents.md", "backlog.md", "CHANGELOG.md", "CLAUDE.md", "design-concept.html",
            "IMPROVEMENTS.md", "PapasBooks.md", "README.md",

            # Legacy & misc
            ".archive", ".kilocode", "docs", "jules-scratch", "legacy", "logs", "public", "tmp",

            # This script and its output
            "tree.txt", "tree_scanner.py"
        }
...
def main():
    ...
    # Setup excludes - organized by category (must match __init__ defaults)
    default_excludes = {
        # Version control & CI/CD
        ".git", ".github", ".codeql", ".codeql-db", "codeql-results.sarif",
